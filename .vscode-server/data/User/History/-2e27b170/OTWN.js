  ...
  try {
    ...
    parties.forEach((party) => {
      ...
      detailsButton.addEventListener('click', async (evt) => {
        // your code here
        evt.preventDefault()
        const renderAllParties = await renderparties(party.id)
        partyContainer.innerHTML = `
                <h2>${renderAllParties.name}</h2>
                <p>${renderAllParties.descrition}</p>
                <p>${renderAllParties.date}</p>
                <p>${renderAllParties.time}</p>
                <p>${renderAllParties.location}</p>
                `
